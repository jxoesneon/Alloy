...
=======
  it("rejects request when token limit exceeded", async () => {
    ...
    expect(auditLogger.log).toHaveBeenCalledWith(
      expect.objectContaining({ type: AuditEventType.RATE_LIMITED }),
    );
  });
  ...
  it("rejects request when request limit exceeded", async () => {
    ...
    expect(jsonMock).toHaveBeenCalledWith(
      expect.objectContaining({
        error: "Budget limit exceeded",
        violations: ["request limit exceeded"],
      }),
    );
  });
  ...
  it("rejects request when cost limit exceeded", async () => {
    ...
    await middleware(req as Request, res as Response, next);
    (res.json as any)({
      llmUsage: { inputTokens: 10000, outputTokens: 10000, model: "gpt-4" },
    }); // expensive
  ...
  it("configures budget based on tenant tier", async () => {
    ...
    req.headers!["x-tenant-id"] = "growth-tenant";
    await middleware(req as Request, res as Response, next);
    expect(getBudgetStatus("growth-tenant")?.config.maxTokensPerDay).toBe(
      500000,
    );

    req.headers!["x-tenant-id"] = "starter-tenant-2";
    await middleware(req as Request, res as Response, next);
    expect(getBudgetStatus("starter-tenant-2")?.config.maxTokensPerDay).toBe(
      100000,
    );
  });
