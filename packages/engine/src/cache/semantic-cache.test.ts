...
>>>>>>> 35868da (chore: final cleanup and enterprise alignment)
  ...
  it("caches PUBLIC outputs with 300s TTL", async () => {
    ...
    const toolOutputs = {
      weather: { result: "sunny", classification: "PUBLIC" },
    };
  ...
  it("caches INTERNAL outputs with 60s TTL", async () => {
    ...
    const toolOutputs = {
      getUserData: { result: "data", classification: "INTERNAL" },
    };
  ...
  it("never caches RESTRICTED outputs", async () => {
    ...
    const toolOutputs = {
      sensitiveData: { result: "secret", classification: "RESTRICTED" },
    };
...
=======
  ...
  it("INTERNAL cache is user-scoped (different key per user)", async () => {
    ...
    const toolOutputs = {
      profile: { result: "private", classification: "INTERNAL" },
    };
    await cache.set(
      "my profile",
      [],
...
=======
  it("invalidate by toolName and params", async () => {
    ...
    await cache.invalidate("weather", { city: "Paris" }); // Should not invalidate
    expect(
      await cache.get("london weather", [], "u1", toolOutputs),
    ).toBeDefined();
  ...
  it("invalidatePattern removes matching tools", async () => {
    const layout = makeLayout("inv-pattern-layout");
    const toolOutputs = {
      "db.query.users": { result: "data", classification: "PUBLIC" },
    };
    await cache.set("users", [], "u1", toolOutputs, layout, "PUBLIC");
