...
const CACHE_SIGNING_SECRET =
  process.env.CACHE_SIGNING_SECRET ??
  "ferroui-cache-hmac-secret-CHANGE-IN-PRODUCTION";
...
export class SemanticCache {
  ...
  private resolveClassification(
    ...
    for (const output of Object.values(toolOutputs)) {
      const c = (output as Record<string, unknown>)?.classification as
        | string
        | undefined;
  ...
  public async get(
    ...
    if (!this.verifyEntry(entry)) {
      console.error(
        "[SemanticCache] HMAC verification failed — possible cache poisoning. Evicting entry.",
      );
      await this.backend.delete(key);
      this.usageOrder.delete(key);
      return undefined;
  ...
  public async invalidate(toolName: string, params?: unknown): Promise<void> {
    ...
    for (const key of keys) {
      ...
      if (output) {
        ...
        if (
          (output as Record<string, unknown>).args &&
          JSON.stringify((output as Record<string, unknown>).args) ===
  ...
  private generateKey(
    ...
    const permissionsHash = CryptoJS.SHA256(sortedPermissions.join(","))
      .toString()
      .slice(0, 32);
