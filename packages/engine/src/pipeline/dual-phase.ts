...
export async function* runDualPhasePipeline(
  ...
  yield {
    type: "phase",
    phase: 1,
    content: "Starting Phase 1: Data Gathering",
  };
  ...
  if (toolCalls.length > MAX_TOOL_CALLS_PER_REQUEST) {
    yield {
      ...
      error: {
        ...
        message: `LLM requested ${toolCalls.length} tool calls; budget is ${MAX_TOOL_CALLS_PER_REQUEST}. Truncating.`,
        retryable: false,
      },
    };
  }
  ...
  for (const call of budgetedToolCalls) {
    ...
    try {
      ...
      yield {
        type: "tool_output",
        toolOutput: { name: call.name, result: toolOutputs[call.name] },
      };
      auditLogger.log({
        type: AuditEventType.TOOL_CALL,
        requestId: context.requestId,
  ...
  if (config.cacheEnabled && !shouldBypassCache) {
    const cachedLayout = await semanticCache.get(
      prompt,
      context.permissions,
      context.userId,
      cacheToolOutputs,
  ...
  const phase2InputEstimate = providerRef.current.estimateTokens(
    phase2SystemPrompt + prompt,
  );
  ...
  if (!dailyBudgetStore.checkBudget(tenantId, phase2CostEstimate)) {
    yield {
      ...
      error: {
        ...
        message:
          "Insufficient daily budget to proceed with UI generation phase.",
        retryable: false,
      },
    };
    return;
  ...
  if (phase2Response) {
    const phase2ActualCost = providerRef.current.estimateCost(
      phase2Response.tokens,
    );
