...
export class ProviderRouter implements LlmProvider {
  ...
  private isHealthy(providerId: string): boolean {
    ...
    if (Date.now() - h.lastFailureAt > CIRCUIT_RESET_MS) {
      ...
      this.health.set(providerId, h);
      console.info(
        `[ProviderRouter] Circuit auto-reset for provider "${providerId}"`,
      );
      return true;
    }
    return false;
  ...
  async *processPrompt(
    ...
    for (const candidate of candidates) {
      try {
        ...
        console.warn(
          `[ProviderRouter] Provider "${candidate.provider.id}" failed during stream:`,
  ...
  async completePrompt(req: LlmRequest): Promise<LlmResponse> {
    ...
    for (const candidate of candidates) {
      try {
        ...
        console.warn(
          `[ProviderRouter] Provider "${candidate.provider.id}" failed: ${msg}`,
        );
        this.recordFailure(candidate.provider.id);
    ...
    throw new Error(
      `[ProviderRouter] All providers failed: ${errors.join(" | ")}`,
    );
  }
