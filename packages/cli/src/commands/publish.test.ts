...
=======
  it("should throw an error if component path does not exist", async () => {
    ...
    await expect(
      publishCommand("/fake/path", { dryRun: false }),
    ).rejects.toThrow("Component path does not exist: /fake/path");
  });
  ...
  it("should publish successfully to the default registry", async () => {
    ...
    fetchMock.mockResolvedValue({
      ok: true,
      json: async () => ({
        url: "https://registry.ferroui.dev/components/Button",
        id: "123",
      }),
    });
  ...
  it("should publish successfully to a custom registry", async () => {
    ...
    fetchMock.mockResolvedValue({
      ok: true,
      json: async () => ({
        url: "https://custom.registry/components/Button",
        id: "456",
      }),
    });

    await publishCommand("/fake/path", {
      dryRun: false,
