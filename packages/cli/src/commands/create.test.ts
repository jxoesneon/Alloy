...
describe("createCommand", () => {
  ...
  it("should exit if directory already exists", async () => {
    ...
    await expect(
      createCommand.parseAsync(["node", "create", "my-app"]),
  ...
  it("should create project with defaults when no options passed", async () => {
    ...
    expect(fs.writeFile).toHaveBeenCalledWith(
      expect.stringContaining("ferroui.config.ts"),
      expect.any(String),
    );
  ...
  it("should handle install failure gracefully", async () => {
    ...
    expect(oraWarnMock).toHaveBeenCalledWith(
      expect.stringContaining('Could not run "pnpm install"'),
    );
  });
  ...
  it("should exit on file system error", async () => {
    ...
    await expect(
      createCommand.parseAsync(["node", "create", "my-app", "-t", "minimal"]),
    ).rejects.toThrow("Exit 1");
