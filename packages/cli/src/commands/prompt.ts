...
const ENGINE_PINNED_VERSION_FILE = path.join(
  resolveRoot(),
  "packages/engine/.prompt-version",
);

export const promptCommand = new Command("prompt").description(
...
promptCommand
  ...
  .action(async () => {
    ...
    try {
      const entries = await fs.readdir(ENGINE_PROMPTS_PATH, {
        withFileTypes: true,
      });
      ...
      if (versions.length === 0) {
        ...
        versions.forEach((v) => {
          ...
          console.log(
            `  ${isPinned ? chalk.cyan("*") : " "} v${v}${isPinned ? chalk.dim(" (pinned)") : ""}`,
          );
        });
      }
    } catch (error: any) {
...
promptCommand
  ...
  .action(async (v1, v2) => {
    ...
    try {
      ...
      console.error(
        chalk.red(`Error: One or both versions (v${v1}, v${v2}) do not exist.`),
      );
      process.exit(1);
    }
    ...
    try {
      ...
      const diffProcess = spawnSync("diff", ["-u", "-r", dir1, dir2], {
        encoding: "utf-8",
      });
      if (diffProcess.stdout) {
        console.log(diffProcess.stdout);
      }
      if (diffProcess.status === 1 && !diffProcess.stdout) {
        // Differences found but no stdout? Unexpected but possible.
        console.log(
          chalk.yellow("Differences found but could not be displayed."),
        );
      }
    } catch (error: any) {
      console.error(
        chalk.red('Failed to run diff. Ensure "diff" utility is installed.'),
      );
      console.error(chalk.dim(error.message));
    }
  });
...
promptCommand
  ...
  .action(async (version) => {
    const spinner = ora(
      `Pinning engine to prompt version v${version}...`,
    ).start();
    ...
    try {
      ...
      spinner.fail(
        chalk.red(
          `Version v${version} does not exist in ${ENGINE_PROMPTS_PATH}`,
        ),
      );
      process.exit(1);
    }
    ...
    try {
      ...
      spinner.succeed(
        chalk.green(
          `Engine successfully pinned to prompt version ${chalk.bold(version)}`,
        ),
      );
      console.log(
        chalk.dim(`Pinned version stored in: ${ENGINE_PINNED_VERSION_FILE}`),
      );
    } catch (error: any) {
