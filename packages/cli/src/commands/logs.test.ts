...
describe("logsCommand", () => {
  ...
  it("should fetch recent logs for default engine service", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:4000/admin/logs?lines=50",
    );
  ...
  it("should fetch recent logs with json output", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:4000/admin/logs?lines=10",
    );
  ...
  it("should handle fetch failure for recent logs", async () => {
    ...
    expect(consoleErrorMock).toHaveBeenCalledWith(
      expect.stringContaining("Failed to fetch logs: HTTP 404"),
    );
    expect(processExitMock).toHaveBeenCalledWith(1);
  });
  ...
  it("should handle fetch exception for recent logs", async () => {
    ...
    expect(consoleErrorMock).toHaveBeenCalledWith(
      expect.stringContaining("Cannot connect to service: Network error"),
    );
    expect(processExitMock).toHaveBeenCalledWith(1);
  });
  ...
  it("should stream logs when follow is true", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:3002/admin/logs?stream=true",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      expect.stringContaining("Streaming logs..."),
    );
  ...
  it("should handle stream connection errors", async () => {
    ...
    expect(consoleErrorMock).toHaveBeenCalledWith(
      expect.stringContaining("Stream error: HTTP 500"),
    );
  });
});
