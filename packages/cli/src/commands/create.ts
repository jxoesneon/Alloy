...
const TEMPLATES: Record<string, { label: string; description: string }> = {
  default: {
    label: "Default",
    description: "React + TypeScript + Node.js engine",
  },
  minimal: { label: "Minimal", description: "Core packages only, no examples" },
  full: {
    label: "Full",
    description: "Examples, Storybook, and documentation",
  },
};
...
export const createCommand = new Command("create")
  ...
  .action(async (name: string, options: Record<string, any>) => {
    ...
    const pkgManager: string =
      options.pkgManager || answers.pkgManager || "pnpm";

    const allowedPkgManagers = ["npm", "yarn", "pnpm"];
    if (!allowedPkgManagers.includes(pkgManager)) {
      console.error(chalk.red(`✖ Invalid package manager: ${pkgManager}`));
      process.exit(1);
    }
...
`,
      ...
      if (useGit) {
        ...
        try {
          ...
          spawnSync(
            "git",
            ["commit", "-m", "chore: initial commit from ferroui create"],
          ...
          gitSpinner.warn(
            chalk.yellow(
              "Could not initialize git (git not found or no config)",
            ),
          );
        }
      }
      ...
      if (runInstall) {
        ...
        try {
          ...
          installSpinner.warn(
            chalk.yellow(
              `Could not run "${pkgManager} install" — run it manually`,
            ),
          );
        }
      }
