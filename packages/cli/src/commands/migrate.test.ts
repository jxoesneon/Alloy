...
describe("migrateCommand", () => {
  ...
  describe("component type", () => {
    it("should exit if --from and --to are not provided", async () => {
      await expect(
        migrateCommand.parseAsync(["node", "migrate", "component"]),
      ).rejects.toThrow("Exit 1");

      expect(consoleErrorMock).toHaveBeenCalledWith(
        expect.stringContaining("requires --from and --to versions"),
      );
    });
    ...
    it("should exit if target component is not found", async () => {
      ...
      expect(consoleErrorMock).toHaveBeenCalledWith(
        expect.stringContaining("Target component not found"),
      );
    });
    ...
    it("should migrate component successfully", async () => {
      vi.mocked(registry.getComponentEntry)
        ...
        .mockReturnValueOnce({
          schema: { shape: { title: true, subtitle: true } },
        } as any); // to
      ...
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("+ New properties: subtitle"),
      );
    });
    ...
    it("should handle validation failure", async () => {
      ...
      expect(fs.writeJson).not.toHaveBeenCalled();
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("Error: Validation failed: Invalid prop"),
      );
    });
    ...
    it("should migrate component and log removed properties", async () => {
      ...
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("- Removed properties: subtitle"),
      );
    });
    ...
    it("should skip layout file if no matching components are found", async () => {
      ...
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("No matching components found"),
      );
    });
    ...
    it("should catch error when readJson fails for component migrate", async () => {
      ...
      vi.mocked(fs.readJson).mockRejectedValue(
        new Error("Component migrate read fail"),
      );
      ...
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("Error: Component migrate read fail"),
      );
    });
  });
  ...
  describe("layout type", () => {
    it("should catch error when readJson fails for auto-migrate", async () => {
      ...
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("Error: Failed to read json"),
      );
    });
    ...
    it("should skip layout file if no deprecated components are found", async () => {
      ...
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("No deprecated components found"),
      );
    });
    ...
    it("should handle validation failure during auto-migration", async () => {
      ...
      vi.mocked(validateLayout).mockReturnValue({
        valid: false,
        errors: [{ message: "Auto migrate valid fail" }],
      } as any);
      ...
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("Auto migrate valid fail"),
      );
    });
    ...
    it("should traverse safely with missing component type and duplicate components", async () => {
      ...
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("OldCard → NewCard"),
      );
      expect(consoleLogMock).toHaveBeenCalledWith(
        expect.stringContaining("NoReplacementCard (no replacement available)"),
      );
    });

    it("should output unknown migration type", async () => {
      await expect(
        migrateCommand.parseAsync(["node", "migrate", "unknown-type"]),
      ).rejects.toThrow("Exit 1");

      expect(consoleErrorMock).toHaveBeenCalledWith(
        expect.stringContaining("Unknown migration type: unknown-type"),
      );
    });
  });
});
