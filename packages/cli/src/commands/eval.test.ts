...
describe("evalCommand", () => {
  ...
  it("should load custom evals from directory as single object", async () => {
    ...
    fetchMock.mockResolvedValue({
      ok: true,
      text: async () =>
        'data: {"type": "layout_chunk", "layout": {"schemaVersion": "1.0", "layout": {"type": "Dashboard", "children": [{}], "aria": {}}}}\n',
    });
  ...
  it("should fail eval if engine returns error", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      expect.stringContaining("0/1 passed"),
    );
  });
  ...
  it("should fail eval if schema is invalid", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      text: async () =>
        'data: {"type": "layout_chunk", "layout": "invalid layout"}\n',
    });
  ...
  it("should exit with 1 if CI flag is passed and pass rate is below threshold", async () => {
    ...
    await expect(
      evalCommand.parseAsync(["node", "eval", "--prompt", "Test", "--ci"]),
  ...
  it("should handle chunk type error", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      text: async () =>
        'data: {"type": "error", "error": {"message": "Custom error message"}}\n',
    });
