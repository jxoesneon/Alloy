...
describe("buildCommand", () => {
  ...
  it("should successfully run a full build", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      expect.stringContaining("FerroUI Production Build"),
    );
  ...
  it("should abort if type check fails", async () => {
    ...
    expect(consoleErrorMock).toHaveBeenCalledWith(
      expect.stringContaining("Build aborted due to type errors."),
    );
  ...
  it("should handle package build failure", async () => {
    ...
    expect(consoleErrorMock).toHaveBeenCalledWith(
      expect.stringContaining("Build completed with errors."),
    );
    expect(processExitMock).toHaveBeenCalledWith(1);
  });
