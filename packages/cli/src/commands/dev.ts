...
export const devCommand = new Command("dev")
  .description(
    "Start the development environment (engine, registry inspector, and playground).",
  )
  ...
  .option(
    "--renderer-only",
    "Start only the layout playground/renderer (frontend)",
  )
  .action(async (options) => {
    ...
    const startEngine =
      options.engineOnly || (!options.engineOnly && !options.rendererOnly);
    const startRenderer =
    ...
    try {
      ...
      if (startRenderer) {
        ...
        if (webExists) {
          ...
          spinner.warn(
            chalk.yellow("apps/web not found — playground not started"),
          );
        }
      }
      ...
      if (startEngine) {
        ...
        if (engineEntry) {
          ...
          spinner.warn(
            chalk.yellow("Engine entry not found — engine not started"),
          );
        }
        ...
        const inspectorDist = path.join(
          packagesDir,
          "registry/dist/inspector.js",
        );
        const inspectorSrc = path.join(
          packagesDir,
          "registry/src/inspector.ts",
        );
        ...
        if (inspectorEntry) {
          ...
          spinner.warn(
            chalk.yellow(
              "Registry inspector entry not found — inspector not started",
            ),
          );
        }
      }
