...
generateCommand
  ...
  .description(
    "Generate a new FerroUI component with Zod schema, types, index, stories, and tests.",
  )
  .option(
    "-t, --tier <tier>",
  ...
  .action(async (name, options) => {
    ...
    try {
      ...
      spinner.succeed(
        chalk.green(
          `Generated ${chalk.bold(componentName)} in ${options.dir}/${componentName}`,
        ),
      );
...
generateCommand
  .command("tool <name>")
  .description(
    "Generate a new FerroUI tool with Zod schema, execute function, and mock.",
  )
  .option(
    "-c, --category <category>",
    "Tool category (data-fetch, computation, external-api)",
    "data-fetch",
  )
  ...
  .action(async (name, options) => {
    ...
    try {
      ...
      spinner.succeed(
        chalk.green(
          `Generated ${chalk.bold(toolName)} in ${options.dir}/${toolName}`,
        ),
      );
...
generateCommand
  ...
  .action(async (name, options) => {
    ...
    try {
      ...
      spinner.succeed(
        chalk.green(
          `Generated ${chalk.bold(promptName)} in ${options.dir}/${version}/${promptName}.md`,
        ),
      );
    } catch (error: any) {
