...
describe("deployCommand", () => {
  ...
  describe("Web Target", () => {
    it("should deploy to Vercel if available", async () => {
      vi.mocked(execa).mockImplementation(((cmd: any) => {
        if (cmd === "vercel")
          return Promise.resolve({ all: "https://vercel.com" } as any);
      ...
      expect(oraSucceedMock).toHaveBeenCalledWith(
        expect.stringContaining("Deployed to Vercel!"),
      );
    });
    ...
    it("should deploy to Netlify if Vercel is not available", async () => {
      vi.mocked(execa).mockImplementation(((cmd: any) => {
        ...
        if (cmd === "netlify")
          return Promise.resolve({ all: "https://netlify.com" } as any);
        return Promise.resolve({} as any);
      }) as any);
      ...
      expect(execa).toHaveBeenCalledWith(
        "netlify",
        ["deploy", "--dir", expect.any(String), "--prod"],
        expect.any(Object),
      );
      expect(oraSucceedMock).toHaveBeenCalledWith(
        expect.stringContaining("Deployed to Netlify!"),
      );
    });
    ...
    it("should fail if no provider is found", async () => {
      ...
      expect(oraFailMock).toHaveBeenCalledWith(
        expect.stringContaining("No deployment provider found"),
      );
      expect(processExitMock).toHaveBeenCalledWith(1);
    });
    ...
    it("should perform a dry run", async () => {
      ...
      expect(oraSucceedMock).toHaveBeenCalledWith(
        expect.stringContaining("[dry-run] Web deployment validated"),
      );
  ...
  describe("Edge Target", () => {
    it("should fail if wrangler is not found", async () => {
      ...
      expect(oraFailMock).toHaveBeenCalledWith(
        expect.stringContaining("Wrangler CLI not found"),
      );
      expect(processExitMock).toHaveBeenCalledWith(1);
    });
    ...
    it("should deploy using wrangler", async () => {
      vi.mocked(execa).mockImplementation(((cmd: any) => {
        if (cmd === "wrangler")
          return Promise.resolve({ all: "Success" } as any);
        return Promise.resolve({} as any);
      }) as any);
      ...
      expect(oraSucceedMock).toHaveBeenCalledWith(
        expect.stringContaining("Deployed to Cloudflare Workers!"),
      );
    });
  });
  ...
  describe("Desktop Target", () => {
    ...
    it("should build desktop using tauri", async () => {
      ...
      expect(oraSucceedMock).toHaveBeenCalledWith(
        expect.stringContaining("Desktop app built!"),
      );
    });
    ...
    it("should perform a dry run for desktop", async () => {
      ...
      expect(oraSucceedMock).toHaveBeenCalledWith(
        expect.stringContaining("[dry-run] Desktop build config validated"),
      );
      expect(consoleLogMock).toHaveBeenCalledWith(
