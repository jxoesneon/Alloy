...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
      ...
      inputSchema: {
        ...
        properties: {
          prompt: {
            type: "string",
            description:
              'Natural language description of the UI to generate (e.g. "Show me a sales dashboard with KPIs").',
          },
          userId: {
            type: "string",
            description:
              'Caller user ID for permission scoping (default: "mcp-user").',
          },
          permissions: {
            type: "array",
            items: { type: "string" },
            description:
              "Permission strings that control which tools the pipeline can invoke.",
          },
          locale: {
            type: "string",
    ...
    },
    {
      ...
      description:
        "Validate a FerroUI layout JSON object against the schema and tier rules. Returns {valid, errors}.",
      inputSchema: {
        type: "object",
        properties: {
          layout: {
    ...
    },
    {
      ...
      description:
        "List all registered FerroUI backend tools with their descriptions and parameter schemas.",
      inputSchema: {
        type: "object",
        properties: {
          permissions: {
            ...
            items: { type: "string" },
            description:
              "Filter to tools accessible with these permissions (default: all public tools).",
          },
        },
      },
    },
    {
      ...
      description:
        "List all known FerroUI components with their tier classification (Atom/Molecule/Organism).",
      inputSchema: {
        type: "object",
        properties: {},
      },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
          ...
          if (chunk.type === "complete" && chunk.layout) {
            ...
            return {
              content: [
                {
                  type: "text",
                  text: `Pipeline error: ${chunk.error?.message ?? "unknown"}`,
                },
              ],
              isError: true,
            };
    ...
    case "ferroui_list_components": {
      const components = Object.entries(COMPONENT_TIER_REGISTRY).map(
        ([name, tier]) => ({
          name,
          tier,
        }),
  ...
  resources: [
    {
      ...
      description:
        "JSON Schema describing the FerroUILayout object structure and all component types.",
    ...
    },
    {
      ...
      description:
        "Full list of registered FerroUI components with tier classifications.",
      mimeType: "application/json",
    },
  ],
}));
...
export async function main() {
  ...
  console.error(
    `[FerroUI MCP] Server running on stdio (provider: ${process.env.LLM_PROVIDER ?? "anthropic"})`,
  );
}
