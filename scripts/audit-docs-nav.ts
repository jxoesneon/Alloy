...
function getAllMdFiles(dir: string, allFiles: string[] = []): string[] {
  ...
  for (const file of files) {
    ...
    if (
      EXCLUDED_DIRS.some(
        (d) => relativePath === d || relativePath.startsWith(d + "/"),
      )
    )
      continue;
    if (
...
async function main() {
  ...
  const navTs = fs.readFileSync(
    path.join(DOCS_DIR, ".vitepress", "nav.ts"),
    "utf-8",
  );
  const sidebarTs = fs.readFileSync(
  ...
  for (const file of mdFiles) {
    ...
    const foundInNav =
      navTs.includes(`link: '${file}'`) || navTs.includes(`link: "${file}"`);
    const foundInSidebar =
      sidebarTs.includes(`link: '${file}'`) ||
      sidebarTs.includes(`link: "${file}"`);
