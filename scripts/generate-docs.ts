...
export function describeType(node: JsonSchemaNode | undefined): string {
  ...
  if (Array.isArray(node.oneOf))
    return node.oneOf.map(describeType).join(" \\| ");
  if (Array.isArray(node.allOf))
    return node.allOf.map(describeType).join(" & ");
  ...
  for (const [key, prop] of Object.entries(node.properties)) {
    ...
    const def =
      prop.default !== undefined
        ? codeInline(JSON.stringify(prop.default))
        : "—";
    rows.push(
...
function renderJsonExample(entry: RegistryEntry): string {
  ...
  if (definition.properties) {
    for (const [key, prop] of Object.entries(definition.properties) as Array<
...
async function writeComponentsPage(): Promise<void> {
  ...
  for (const tier of ["Organism", "Molecule", "Atom"]) {
    tierSummary.push(
      `| ${tierBadge(ComponentTier[tier.toUpperCase() as keyof typeof ComponentTier])} | ${byTier[tier]?.length ?? 0} |`,
    );
  }
...
async function writeToolsPage(): Promise<void> {
  ...
  publicTools.sort((a: ToolDefinition, b: ToolDefinition) =>
  ...
  for (const tool of publicTools) {
    ...
    const badges: string[] = [];
    if (tool.sensitive)
      badges.push("🔒 **Sensitive** (bypasses semantic cache)");
...
const PACKAGE_METADATA: Array<{
  ...
  },
  {
    pkg: "i18n",
    title: "@ferroui/i18n",
    description: "Locale resolution, RTL mirroring, and translation provider.",
    path: "packages/i18n",
  },
  {
    pkg: "telemetry",
    title: "@ferroui/telemetry",
    description: "OpenTelemetry tracing, metrics, and PII redaction pipelines.",
    path: "packages/telemetry",
  },
  {
    pkg: "tokens",
    title: "@ferroui/tokens",
    description: "Design token pipeline with Style Dictionary outputs.",
    path: "packages/tokens",
  },
  {
    pkg: "renderer",
    title: "@ferroui/renderer",
    description: "React renderer with action router and state machine bridge.",
  ...
  },
  {
    ...
    description: "Internal utilities shared across FerroUI packages.",
    path: "packages/shared",
  },
  {
    pkg: "mcp-server",
    title: "@ferroui/mcp-server",
    description: "Model Context Protocol server exposing FerroUI capabilities.",
    path: "packages/mcp-server",
  },
  {
    pkg: "vscode-extension",
    title: "@ferroui/vscode-extension",
    description: "VS Code editor integration with live layout preview.",
    path: "packages/vscode-extension",
  },
];

const APP_METADATA: Array<{
  app: string;
  title: string;
  description: string;
  path: string;
  ...
  {
    app: "web",
    title: "Web (Reference Renderer)",
    description: "React reference renderer used for Storybook, E2E, and demos.",
    path: "apps/web",
  },
  {
    app: "edge",
    title: "Edge (Cloudflare Worker)",
    description: "Cloudflare Worker packaging of the FerroUI engine.",
    path: "apps/edge",
  },
  {
    app: "desktop",
    title: "Desktop",
    description:
      "Tauri-based desktop shell for offline FerroUI runtimes (experimental).",
    path: "apps/desktop",
  },
  {
    app: "playground",
    title: "Playground",
    description: "Interactive layout playground (experimental).",
