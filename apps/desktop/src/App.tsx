...
import { useFerroUIStream, FerroUIRenderer } from "@ferroui/renderer";
import {
  actionRouter,
  type ActionRouterContext,
} from "../../web/src/services/ActionRouter.js";
...
function App() {
  const { layout, loading, error, send } = useFerroUIStream({
    endpoint: "http://localhost:3001/api/layout", // Desktop app connects to local engine
  });

  const refresh = () =>
    send("Show standard diagnostic dashboard", {
      userId: "desktop-user",
  ...
  return (
    <div className="ferroui-app min-h-screen relative">
      ...
      {layout != null && <FerroUIRenderer layout={layout} />}
