...
app.post("/api/layout", async (c) => {
  ...
  try {
    const fullLayout = await synthesizeLayout(
      prompt || "Show standard diagnostic dashboard",
      apiKey,
      logger,
    );
    ...
    return streamSSE(c, async (stream) => {
      ...
      const chunk2 = jsonString.slice(
        Math.floor(jsonString.length / 3),
        Math.floor((jsonString.length * 2) / 3),
      );
...
app.post("/api/tools/call", async (c) => {
  try {
    ...
    if (!response.ok) {
      return c.json(
        { success: false, error: `Engine responded ${response.status}` },
        502,
      );
    }
